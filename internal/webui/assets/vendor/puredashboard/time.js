...
class PuredashboardTime extends Reactive {
  ...
  static observedAttributes = ["value", "min", "max", "step", "size", "disabled", "required", "readonly", "name", "aria-label", "aria-labelledby"];
  ...
  render() {
    ...
    return html`
      <input class="puredashboard-time__field js-puredashboard-time__field${sizeCls}" type="time" aria-label="${this._ariaName()}" aria-labelledby="${this._ariaNamedBy()}" .value="${this.value ?? ""}" min="${this.min ?? ""}" max="${this.max ?? ""}" step="${this.step ?? ""}" ?disabled="${!!this.disabled}" ?required="${!!this.required}" ?readonly="${!!this.readonly}" aria-invalid="${invalid ? "true" : "false"}" aria-describedby="${this.error ? this._errId : ""}">
